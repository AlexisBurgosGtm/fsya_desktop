...
Public Class ViewDocumentos

    Private Sub ViewDocumentos_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Function tblDocumentos(ByVal tipodoc As String) As DataTable
        ...
        Select Case tipodoc
            ...
            Case "IEF"
                sql = "SELECT           CODDOC, CORRELATIVO, FECHA, SUCURSAL_ING, EMPLEADO, MONTO, HORA, MINUTO, MOTORISTA, NO_CORTE, OBSERVACIONES
                       FROM             EFECTIVO_KH
                       WHERE            (MES = @MES) AND (ANIO = @ANIO) AND (EMPNIT = @EMPNIT)"
...
#Region " *** RADIAL MENU *** "
        ...
        Select Case GlobalTipoDoc
            ...
            Case "FEF"
                ...
                If SelectedUUDI <> "" Then
                    ...
                    Else
                        Dim fel As New classFEL
                    fel.rptFacturaFELCONTINGENCIA(SelectedCoddoc, SelectedCorrelativo, 2)
                    'If AbrirReporte_Ticket(SelectedFecha, SelectedCoddoc, SelectedCorrelativo, 2) = True Then
                    '   Else
                    '      MsgBox("No se pudo cargar el documento. Error :" & GlobalDesError)
                    ' End If
            ...
            Case "COR"
                ...
                'Call Report_ProductosCorteEXISTENCIAS(GlobalEmpNit, SelectedCorrelativo, 1)
                Call ExportMovimientos()
    ...
    Private Sub ExportMovimientos()

        Me.gridExports1.DataSource = Nothing
        Me.gridExports1.DataSource = TblMovinCorte(GlobalEmpNit, SelectedCorrelativo)
        Try
            Me.gridExports1.ExportToXlsx(Application.StartupPath + "\EXPORTS\CORTE_" + SelectedCorrelativo.ToString + "_DE_" + GlobalNomEmpresa.ToString + ".xlsx")
            Process.Start(Application.StartupPath + "\EXPORTS\CORTE_" + SelectedCorrelativo.ToString + "_DE_" + GlobalNomEmpresa.ToString + ".xlsx")
                ...
                If drw.Item(16).ToString <> "" Then
                    ...
                    If Confirmacion("¿Está seguro que desea Anular un Documento Electrónico?", Me.ParentForm) = True Then
                        ...
                        If FlyoutDialog.Show(Me.ParentForm, New ViewClave(Form1.txtConfigClaveVerificaExistencia.Text)) = DialogResult.OK Then
                            ...
                            If insertBitacora("ANULACION", SelectedCoddoc, SelectedCorrelativo.ToString) = True Then
                                Form1.Mensaje("Bitacora actualizada")
                            Else
                                Exit Sub
                            End If
...
ANULACION_FAC:
    ...
    Private Function insertBitacora(ByVal doc As String, ByVal coddoc As String, ByVal corr As String) As Boolean
        ...
        Using cn As New SqlConnection(strSqlConectionString)
            ...
            Try
                ...
                Dim cmd As New SqlCommand("INSERT INTO BITACORA (FECHA, HORA, MINUTO, USUARIO, MODULO, ACCION) VALUES (@FECHA, @HORA, @MINUTO, @USUARIO, @MODULO, @ACCION)", cn)
                'cmd.CommandType = CommandType.StoredProcedure
                ...
                cmd.Parameters.AddWithValue("@MODULO", doc)
                Select Case doc
                    Case "ELIMINACION"
                        cmd.Parameters.AddWithValue("@ACCION", "Documento " + coddoc + ", correlativo " + corr + " eliminado.")
                    Case "ANULACION"
                        cmd.Parameters.AddWithValue("@ACCION", "Documento " + coddoc + ", correlativo " + corr + " anulado.")
                End Select
                cmd.ExecuteNonQuery()
                result = True
    ...
    Private Sub BarButRadMenListadosEliminar_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles BarButRadMenListadosEliminar.ItemClick
        ...
        If Confirmacion("¿Está seguro que desea eliminar este Documento?", Me.ParentForm) = True Then
            ...
            If FlyoutDialog.Show(Me.ParentForm, New ViewClave("razors1805")) = DialogResult.OK Then
                ...
                If insertBitacora("ELIMINACION", SelectedCoddoc, SelectedCorrelativo.ToString) = True Then
                    Form1.Mensaje("Bitacora actualizada")
                Else
                    Exit Sub
                End If
